...
if (empty($_SESSION["adm_id"]) || (isset($_SESSION['last_activity']) && (time() - $_SESSION['last_activity'] > 600))) {
  ...
  {
    $form_id = htmlspecialchars($db, $_GET['form_id']);
    $status = htmlspecialchars($db, $_POST['status']);
    $remark = htmlspecialchars($db, $_POST['remark']);

    // Prepare statement for insert
    $stmt = $db->prepare("INSERT INTO remark (frm_id, status, remark) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $form_id, $status, $remark);
    $stmt->execute();
...
<?php 
if (isset($_GET['newform_id'])) {
  $newform_id = mysqli_real_escape_string($db, $_GET['newform_id']);

  // Prepare statement for users_orders
  $stmt = $db->prepare("SELECT * FROM users_orders WHERE o_id=?");
  $stmt->bind_param("s", $newform_id);
  $stmt->execute();
