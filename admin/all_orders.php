...
if ($result->num_rows > 0) {
    while ($rows = $result->fetch_assoc()) {
        ...
        <tr>
            ...
            <td>$<?php echo htmlspecialchars($rows['price']); ?></td>
...
?>
		<?php
$sql = "SELECT DATE_FORMAT(date, '%Y-%m-%d') as date, o_id FROM users_orders;"; 
$query = $db->prepare($sql);
$query->execute();
$result = $query->get_result();
...
if ($result->num_rows > 0) {
    while ($rows = $result->fetch_assoc()) {
        ...
        <td>
            <a href="delete_orders.php?order_del=<?php echo htmlspecialchars($rows['o_id']); ?>" onclick="return confirm('Are you sure?');" class="btn btn-danger btn-flat btn-addon btn-xs m-b-10"><i class="fa fa-trash-o" style="font-size:16px"></i></a>
            <a href="view_order.php?user_upd=<?php echo htmlspecialchars($rows['o_id']); ?>" class="btn btn-info btn-flat btn-addon btn-sm m-b-10 m-l-5"><i class="fa fa-edit"></i></a>
        </td>
        </tr>
<?php
    }
