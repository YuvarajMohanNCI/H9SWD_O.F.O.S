...
if (strlen($_SESSION['user_id']) == 0) {
  ...
  try {
    if (isset($_POST['update'])) {
      ...
      $insertRemarkQuery = "INSERT INTO remark (frm_id, status, remark) VALUES (?, ?, ?)";
      $stmt1 = $db->prepare($insertRemarkQuery);
      $stmt1->bind_param("iss", $form_id, $status, $remark);
      $stmt1->execute();
      $stmt1->close();

      $updateOrderQuery = "UPDATE users_orders SET status = ? WHERE o_id = ?";
      $stmt2 = $db->prepare($updateOrderQuery);
