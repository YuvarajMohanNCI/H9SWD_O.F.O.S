...
// Check if the user is not logged in and the session timeout has expired
...
<body class="home">
    ...
    $stmt = $db->prepare("SELECT * FROM dishes LIMIT 6");
if ($stmt === false) {
...
while ($r = $result->fetch_assoc()) {
    ...
    if ($ordering_allowed) {
        echo '<a href="dishes.php?res_id='.$r['rs_id'].'" class="btn theme-btn-dash pull-right">Order Now</a>';
    } else {
        echo '<button class="btn theme-btn-dash pull-right" disabled>Order Now</button>';
    }
...
?>
    ...
    $stmt1 = $db->prepare("SELECT * FROM restaurant");
    if ($stmt1 === false) {
        header("Location: error_page.php");
        die("Prepare failed: " . $db->error);
    }
    ...
    while ($rows = $result1->fetch_assoc()) {
        ...
        $stmt2 = $db->prepare("SELECT * FROM res_category WHERE c_id = ?");
        if ($stmt2 === false) {
            header("Location: error_page.php");
            die("Prepare failed: " . $db->error);
        }
        $stmt2->bind_param("i", $rows['c_id']);
